...
let displayedText = ''; 
let showText = false; 
let mouseXClicked, mouseYClicked;
...
function preload() {
    ...
    randomTexts.push('ART MUST BE DESTRUCTIVE');
    randomTexts.push('AND CONSTRUCTIVE');
    randomTexts.push('WE LOVE INSTABILITY AND CHAOS');
    randomTexts.push('BECAUSE THEY STAND FOR PROGRESS');
    randomTexts.push('');
}
...
function draw() {
    ...
    //Random text
    if (showText) {
          displayRandomText(mouseXClicked, mouseYClicked);
}
}

function displayRandomText(x, y) {
    fill(0);
    textAlign(CENTER);
    textSize(64);
...
function updateFallingImages() {
    for (let i = fallingImages.length - 1; i >= 0; i--) {
        ...
        if (fallingImage.image.position.y > height + 100) {
...
function mousePressed() {
    ...
    } else if (showScissors === false) {
        displayedText = random(randomTexts);
